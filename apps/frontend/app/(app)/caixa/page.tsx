        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="card-sm border-green-200 bg-green-50">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-green-700 font-semibold uppercase tracking-wider">Caixa Aberto</span>
            </div>
          ...
          <div className="card-sm">
            <p className="text-xs text-muted uppercase tracking-wider font-semibold mb-3">Entradas</p>
            <p className="stat-value text-green-600">R$ {totalIncome.toFixed(2).replace('.', ',')}</p>
            <p className="text-xs text-muted mt-1">{MOCK_OPERATIONS.filter(o => o.type === 'income').length} operações</p>
          </div>

          <div className="card-sm">
            <p className="text-xs text-muted uppercase tracking-wider font-semibold mb-3">Sangrias</p>
            <p className="stat-value text-red-500">R$ {totalWithdrawals.toFixed(2).replace('.', ',')}</p>
            <p className="text-xs text-muted mt-1">{MOCK_OPERATIONS.filter(o => o.type === 'withdrawal').length} retirada</p>
          </div>

          <div className="card-sm">
            <p className="text-xs text-muted uppercase tracking-wider font-semibold mb-3">Valor inicial</p>
            <p className="stat-value">R$ 200,00</p>
            <p className="text-xs text-muted mt-1">Fundo de caixa</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 card p-0 overflow-hidden">
            <div className="px-5 py-4 border-b border-border bg-surface">
              <h2 className="text-sm font-semibold text-slate-900">Movimentações do Turno</h2>
            </div>
              ...
              {MOCK_OPERATIONS.map((op) => (
                <div key={op.id} className="flex items-center gap-4 px-5 py-3 hover:bg-surface-2 transition-colors">
                  <div className={cn(
                    ...
                    op.type === 'income' ? 'bg-green-50' : 'bg-red-50',
                  )}>
                    {op.type === 'income'
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-900 truncate">{op.description}</p>
                  </div>
                  <span className="text-xs font-mono text-muted shrink-0">{op.time}</span>
                  <span className={cn(
                    'text-sm font-semibold shrink-0 w-24 text-right',
                    op.type === 'income' ? 'text-green-600' : 'text-red-500',
                  )}>
            ...
            {showSangria ? (
              <div className="card animate-fade-in">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-slate-900">Registrar Sangria</h3>
                  <button onClick={() => setShowSangria(false)} className="text-muted hover:text-slate-900">
                    <X size={14} />
                  </button>
                </div>
                <div className="space-y-3">
