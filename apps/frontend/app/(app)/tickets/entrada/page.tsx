      ...
      <div>
        ...
        <div className="p-6 flex items-center justify-center min-h-[400px]">
          <div className="text-center animate-fade-in">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2">Entrada registrada!</h2>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="card space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  {...register('plate')}
                  className={cn('input font-mono uppercase flex-1', errors.plate && 'border-red-400')}
                  placeholder="ABC1D23"
                  maxLength={8}
                />
                <button
              ...
              <p className="text-xs text-muted mt-1">Formato Mercosul (ABC1D23) ou antigo (ABC-1234)</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  {...register('brand')}
                  className={cn('input', errors.brand && 'border-red-400')}
                >
              ...
              <div>
                ...
                <input
                  {...register('model')}
                  className={cn('input', errors.model && 'border-red-400')}
            ...
            <div>
              ...
              <select
                {...register('color')}
                className={cn('input', errors.color && 'border-red-400')}
              >
                <option value="">Selecionar...</option>
                ...
                {FREE_SPOTS.map((n) => (
                  <label key={n} className="cursor-pointer">
                    ...
                    <div className="aspect-square rounded border text-xs flex items-center justify-center font-mono transition-all
                      ...
                      peer-checked:bg-accent peer-checked:text-black peer-checked:border-accent peer-checked:font-bold
                      hover:border-border-2 hover:text-slate-900">
                      {n}
                    </div>
                  </label>
                ))}
              </div>
              {errors.spot && <p className="text-xs text-red-500 mt-1">{errors.spot.message}</p>}
              <p className="text-xs text-muted">{FREE_SPOTS.length} vagas disponíveis</p>
            </div>
